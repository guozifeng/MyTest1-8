 ...
 * 
 * 1 2 List<Fruit> fruits = new ArrayList<Fruit>(); List<? super Apple> = fruits; 为什么使用通配符标记能行得通？
 * 
 ...
 * 
 * 1 2 3 Apple[] apples = new Apple[1]; Fruit[] fruits = apples; fruits[0] = new Strawberry();
 ...
 * 
 * 1 2 3 List<Apple> apples = new ArrayList<Apple>(); List<? extends Fruit> fruits = apples; fruits.add(new
 * Strawberry()); 这次，代码就编译不过去了！Java编译器会阻止你往一个Fruit list里加入strawberry。在编译时我们就能检测到错误，在运行时就不需要进行检查来确保往列表里加入不兼容的类型了。
 * 即使你往list里加入Fruit对象也不行：
 * 
 * 1 fruits.add(new Fruit()); 你没有办法做到这些。事实上你不能够往一个使用了? extends的数据结构里写入任何的值。
 * 
 * 原因非常的简单，你可以这样想：这个? extends T 通配符告诉编译器我们在处理一个类型T的子类型，但我们不知道这个子类型究竟是什么。因为没法确定，为了保证类型安全，我们就不允许往里面加入任何这种类型的数据。
 ...
 * 我们看到fruits指向的是一个装有Apple的某种超类(supertype)的List。同样的，我们不知道究竟是什么超类，
 * 但我们知道Apple和任何Apple的子类都跟它的类型兼容。既然这个未知的类型即是Apple，也是GreenApple的超类，我们就可以写入：
 * 
 * 1 2 fruits.add(new Apple()); fruits.add(new GreenApple()); 如果我们想往里面加入Apple的超类，编译器就会警告你：
 * 
 * 1 2 fruits.add(new Fruit()); fruits.add(new Object()); 因为我们不知道它是怎样的超类，所有这样的实例就不允许加入。
 * 
...
public class TestPECS {
    ...
    /**
     * 如果你想把对象写入一个数据结构里，使用 ? super 通配符 如果你既想存，又想取，那就别用通配符。
     * 
     */
